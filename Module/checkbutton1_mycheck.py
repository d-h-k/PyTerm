...
def btnresult():
	...
	selstr = " / ".join(result)
	selstr = "your choice :" + selstr 
	messagebox.showinfo(title = "select tech", detail = selstr)

def clrbtn():
	clbtnstr = "starving"
	messagebox.showinfo(title = "hwllo", detail = clbtnstr)
	for x in vargroup:
		x.set(0)
...
vargroup = [ check_A, check_B, check_C ]
...
clbtn = tkinter.Button(frame, text = "clear btn", command = clrbtn)
clbtn.grid(sticky = 'we')
